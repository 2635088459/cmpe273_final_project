...
const CONSUME_QUEUE = 'erasegraph.deletion-requests.search-cleanup';
const ROUTING_KEY_STEP_SUCCEEDED = 'step.succeeded';
const ROUTING_KEY_STEP_FAILED = 'step.failed';
const SERVICE_NAME = 'search_cleanup';
const STEP_NAME = 'search_cleanup';

@Injectable()
export class SearchConsumerService implements OnModuleInit, OnModuleDestroy {
  private readonly logger = new Logger(SearchConsumerService.name);
  ...
  private async connectPostgres() {
    ...
    `);
    this.logger.log('Postgres ready for search_index_documents');
  }
  ...
  private async connectRabbitMQ() {
    ...
    this.logger.log('Connected to RabbitMQ (search cleanup)');
  }
    ...
    await this.consumerChannel.consume(
      ...
      async (msg: any) => {
        ...
        try {
          ...
          this.logger.error('Failed to process search cleanup message', err);
  ...
  private async processDeletion(event: DeletionRequestedEvent) {
    ...
    try {
      ...
      this.logger.log(
        `Removed ${deleted} search_index_documents for subject_id=${subject_id} request_id=${request_id}`,
      );
      ...
      await this.publishFailed({
        ...
        trace_id,
        timestamp: new Date().toISOString(),
        error_message: err.message || 'Search cleanup error',
        error_code: 'SEARCH_CLEANUP_FAILED',
      });
    }
  }
